...
var isSubmit = false,submitType = '',smallLogistics = {};
...
socket2.on('getnewjson', function (msg) {  
    console.log('msg',msg); 
    $('body').toast({message: '数据下载完成 !'});     
    if(msg.桌子对应关系 != undefined) {        
        deskToGX = msg.桌子对应关系;        
    }
    if(msg.后工序总MO列表 != undefined) { 
        console.log(isSubmit,submitType)       
        if(isSubmit) {
            if(submitType=='投料') {
                alert('投料记录写入完成');
            ...
            if(submitType=='报工') {                  
                ...
                //         console.log(t)
                //         printResult += t + "：" + m.t + "（"+ t.不良品数 +"）\n";
                //     })
                // })
                // console.log(printResult)
                // if(sunmiInnerPrinter) {
...
socket.on('postidReturn', function (response) {      
    ...
    // $.ajax({
    //     type : "get",
    //     url : "https://www.kenta.cn:8911/type=getdata?dataname=桌子对应关系",        
    //     async : false,
    ...
    // $.ajax({
    //     type : "get",
    //     url : "https://www.kenta.cn:8911/type=getdata?dataname=后工序总MO列表",        
    //     async : false,
    //     success : function(data){
    //         后工序总MO列表 = data;
    //         $('body').toast({message: '后工序总MO列表已下载 !'});
    //     }
    // });
    // $.ajax({
    //     type : "get",
    //     url : "https://www.kenta.cn:8911/type=getdata?dataname=桌子状态",        
    //     async : false,
    //     success : function(data){
    //         桌子状态 = data;
    //         $('body').toast({message: '桌子状态已下载 !'});
                ...
                if (e.keyCode == 13) {
                    ...
                    _.forEach(deskToGX,function(n,key){
                        if(key==$('input[name="桌子编号"]').val()) {
                            selectedtable = n;
                            selectedtable.上班人员 = _.filter(桌子状态,{桌号:n.桌号}).length>0?_.filter(桌子状态,{桌号:n.桌号})[0].姓名+'('+_.filter(桌子状态,{桌号:n.桌号})[0].工号+')' : '未安排上班人员';
                        }
                    })
                    if(selectedtable=='') {
                        alert('桌号有误');
                        ...
                        if(selectedtable.制程名称 == '重工') {
                            ...
                            $('#MOArea').html(selectedtable.桌号+'——'+selectedtable.制程名称+'('+selectedtable.制程代号+')'+'——'+selectedtable.上班人员)
                        }
            ...
            $('input[name="标签号_input"]').keydown(function (e) {
                if (e.keyCode == 13) {  
                    ...
                    var selected = _.find(后工序总MO列表,{领料单号:{最小包装标签:[{物料标签号:inputvalue}]}});console.log('selected',后工序总MO列表,selected)
                    if(allFields.标签号.split(',').indexOf(inputvalue)>-1) {
                        alert('此标签已存在');
                    }else {  
                        if(selected==undefined) {
                            $.get('https://www.kenta.cn:8911/type=tagfromdetail?dataname='+inputvalue,function(yourbo){                                
                                if(yourbo=='错误标签') {
                                    alert('错误的标签号——'+inputvalue)                                    
                                }else {
                                    alert('标签('+inputvalue+')属于领料单——'+yourbo.生产领料单号+' 确认是否未扫描');
                                } 
                            },'json')                                                       
                        }else {
...
function intervarCheck() {
    setTimeout(function(){
        console.log(333,isSubmit,submitType)
        if(isSubmit) { 
            if(submitType=='投料'||submitType=='重工投料') {
                $.get('https://www.kenta.cn:'+hgx_port+'/type=getdata?dataname=后工序总MO列表',function(n){
                    console.log(3333,n)
                    后工序总MO列表 = n;
                    if(submitType=='投料') {
                        alert('投料记录写入完成');
...
//开工
function tagFeeding() {  
    if(isSubmit==true&&(submitType=='投料'||submitType=='重工投料')) {
        alert('请勿重复上机操作,如换标签上机请退出重进!');
        return false;
    }  
    global_hgxArray=[];//初始化
    ...
    var obj = {};
    //为了以防万一 每次报工都去拿一次 后工序总MO列表
    // $.ajax({
    //     url:'https://www.kenta.cn:'+hgx_port+'/type=getdata?dataname=后工序总MO列表',
    //     async: false,
    //     type: "get",
    //     dataType: "json",
    ...
    if(selected.length==0) {
        ...
        if(displaydata.length>0) {  
            ...
            _.map(displaydata,function(value){
                ...
                tbodystr+='<tr><td><input type="checkbox" name="mono" class="checkchoice" id="'+value.物料标签号+'"></td><td>'+value.物料标签号+'</td><td>'+value.工序名称+'</td><td>'+value.工序代号+'</td><td>'+value.status+'</td></tr>';  
            });
...
function getSearching() {  
    if(isSubmit==true&&(submitType=='恢复'||submitType=='重工恢复')) {
        alert('请勿重复报工操作,如换标签报工请退出重进!!');
        return false;
    }   
    $('#detail_3').html('');
    global_hgxArray=[];//初始化
    ...
    var displayData = [],errortags = [],limittags = [];  
    var mydata;           
    _.forEach(allFields.标签号.split(','),function(o){
        var selected = _.find(桌子状态,{正在生产中产品:[{物料标签号:o}]});  console.log('selected',o,selected)
        ...
        if(selected==undefined) {
            ...
            var myindex = _.findIndex(selected.正在生产中产品, ['物料标签号', o]);
            mydata = selected.正在生产中产品[myindex]; 
            var 是否重工 = deskToGX[mydata.桌号].制程名称 == '重工'?true:false;
            if(moment().diff(moment(mydata.开工时间))/1000<mydata.理论用时*0.5) {
                limittags.push(o);
            }
            var obj = {                        
                制程代号:mydata.工序代号,
