var global_tagArray = [];
var isSubmit = false;
var QueryMO = '',QueryMOpage = '',QueryManualBZ = '';
...
socket.on('postidReturn', function (response) {
    ...
    var out = setInterval(function() {console.log(88,QueryMO,QueryMOpage)
        if(QueryMO!=''&&QueryMOpage!='') {
...
function FormSubmit(e) {
    ...
    var otherSelected = _.filter(QueryMOpage,{制令单号:allFields.关联MO})[0];
    var outertag = moment().format('x');
    if(allFields.包装类型=='中箱'){
        postobj.包规 = parseInt(otherSelected.第2层数量);
        postobj.下层包规 = parseInt(otherSelected.第3层数量);
    }else {
        level = _.filter(QueryManualBZ,{collectedData:{成品标签:global_tagArray[0]}})[0].collectedData.level+1; 
        if(parseInt(otherSelected['第'+(level+1)+'层数量'])==0) {
            alert('当前被包装标签已经是最大包装!');
            return false;
        }        
                
        postobj.包规 = _.filter(QueryManualBZ,{collectedData:{成品标签:global_tagArray[0]}})[0].下层包规;
        postobj.下层包规 = parseInt(otherSelected['第'+(level+2)+'层数量']);        
    }
    if(selected) {
        $('#bigtagpreview').html('');
